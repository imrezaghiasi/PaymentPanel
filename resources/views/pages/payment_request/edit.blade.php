<x-app-layout>
    ...
    <form class="dark:text-gray-300" action="{{route('payment_request.update',$payment_request->id)}}" method="POST" enctype="multipart/form-data">
        @method("PUT")
        @csrf
        <div class="flex flex-row justify-center gap-5 mb-5">
            <div class="mb-4 w-1/2">
                ...
                <input
                    ...
                    type="text"
                    maxlength="10"
                    value="{{$payment_request->national_code}}"
            ...
            <div class="mb-4 w-1/2">
                ...
                <select class="text-center w-full px-4 py-2 dark:bg-gray-600" name="category_id">
                    ...
                    @foreach($categories as $category)
                        <option value="{{$category->id}}" {{$category->id == $payment_request->category_id ? 'selected' : ''}}>{{$category->name}}</option>
                    @endforeach
                </select>
                @error('category_id')
                <div class="text-red-600 mt-1 mb-1">{{ $message }}</div>
        ...
        <div class="flex flex-row justify-center gap-5 mb-5">
            ...
            <div class="mb-4 w-1/2">
                ...
                <input
                    name="shaba_number"
                    type="text"
                    value="{{$payment_request->shaba_number}}"
                    maxlength="24"
                    class="w-full rounded shadow-sm dark:shadow-gray-900 px-4 py-2 dark:bg-gray-700 dark:border-gray-800"
                />
                @error('shaba_number')
        ...
        <div class="flex flex-row mb-5">
            <div class="mb-4 w-1/2">
                ...
                <textarea name="request_description" rows="4"
                          ...
                          placeholder="توضیحات درخواست خود را بنویسید ...">{{$payment_request->request_description}}</textarea>
